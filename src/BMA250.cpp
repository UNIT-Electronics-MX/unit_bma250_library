...
int UBMA250::begin(uint8_t range, uint8_t bw)
{
    return beginI2C(range, bw);
}

// I2C initialization
int UBMA250::beginI2C(uint8_t range, uint8_t bw, uint8_t i2c_addr)
{
    _mode = BMA250_MODE_I2C;
...
// SPI initialization
int UBMA250::beginSPI(uint8_t range, uint8_t bw, uint8_t cs_pin, SPIClass* spi_instance)
{
...
void UBMA250::writeRegister(uint8_t reg, uint8_t value)
{
    if (_mode == BMA250_MODE_I2C) {
        Wire.beginTransmission(I2Caddress);
        Wire.write(reg);
...
uint8_t UBMA250::readRegister(uint8_t reg)
{
    uint8_t value = 0;
...
void UBMA250::readMultipleRegisters(uint8_t reg, uint8_t* buffer, uint8_t length)
{
    if (_mode == BMA250_MODE_I2C) {
        Wire.beginTransmission(I2Caddress);
        Wire.write(reg);
